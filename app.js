(function () {
  ...
  Shared.loadData()
    ...
    .catch(function (err) {
      grid.innerHTML = '<div class="empty"><b>데이터를 불러오지 못했어요</b>recipes.json 파일이 이 페이지와 같은 폴더에 있는지 확인해주세요. (로컬에서 열었다면 <code>python3 -m http.server</code>로 실행해야 fetch가 동작합니다)</div>';
      console.error(err);
    });
